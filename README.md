...
| Global properties | `setProperty` adds a key to every later event in the process — not persisted, the event's own key wins, none are attached to an event they would push past the server's 20-key limit, and keys over 40 characters are refused |
